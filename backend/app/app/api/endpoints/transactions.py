...
from app.core.security import CurrentUser, SyncedUser
from app.db.session import DB
from app.models import TransactionType
from app.schemas.common import NOT_FOUND, UNAUTHORIZED, VALIDATION
...
router = APIRouter(prefix="/api/transactions", tags=["transactions"], responses=UNAUTHORIZED)
...
def list_transactions(
    ...
    return transaction_service.list_page(
...
def create_transaction(body: TransactionCreate, user: CurrentUser, db: DB) -> TransactionOut:
    return TransactionOut.model_validate(transaction_service.create(db, user, body))
...
def get_transaction(transaction_id: uuid.UUID, user: CurrentUser, db: DB) -> TransactionOut:
    return TransactionOut.model_validate(transaction_service.get(db, user, transaction_id))
...
def update_transaction(
    ...
    return TransactionOut.model_validate(transaction_service.update(db, user, transaction_id, body))
...
def delete_transaction(transaction_id: uuid.UUID, user: CurrentUser, db: DB) -> None:
    transaction_service.delete(db, user, transaction_id)
