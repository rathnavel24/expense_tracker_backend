...
router = APIRouter(prefix="/api/recurring", tags=["recurring"], responses=UNAUTHORIZED)
...
def list_recurring(user: CurrentUser, db: DB) -> list[RecurringOut]:
    return [RecurringOut.model_validate(r) for r in recurring_service.list_rules(db, user)]
...
def create_recurring(body: RecurringCreate, user: CurrentUser, db: DB) -> RecurringOut:
    return RecurringOut.model_validate(recurring_service.create(db, user, body))
...
def get_recurring(rule_id: uuid.UUID, user: CurrentUser, db: DB) -> RecurringOut:
    return RecurringOut.model_validate(recurring_service.get(db, user, rule_id))
...
def update_recurring(
    ...
    return RecurringOut.model_validate(recurring_service.update(db, user, rule_id, body))
...
def delete_recurring(rule_id: uuid.UUID, user: CurrentUser, db: DB) -> None:
    recurring_service.delete(db, user, rule_id)
