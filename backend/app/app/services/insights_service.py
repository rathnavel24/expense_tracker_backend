...
from app.utils import month_range, shift_month
...
def monthly(db: Session, user: User, year: int, month: int, today: date) -> MonthlyInsights:
    ...
    by_month = stats.totals_by_month(db, user.id, trend_start, end)
    trend = []
    ...
    return MonthlyInsights.model_validate(
        {
            ...
            "payment_methods": [
                ...
                for method, amount, count in stats.expense_totals_by_payment_method(
