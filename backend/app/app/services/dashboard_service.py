...
from app.utils import month_range, occurrence_in, shift_month
...
def monthly(db: Session, user: User, year: int, month: int, today: date | None) -> MonthlyDashboard:
    ...
    prev_totals = stats.totals_by_type(db, user.id, prev_start, prev_end)
    prev_by_category = {
        category.id: amount
        for category, amount, _ in stats.expense_totals_by_category(
    ...
    if today is not None:
        ...
        day = stats.totals_by_type(db, user.id, today, today + timedelta(1))
    ...
    breakdown = stats.expense_totals_by_category(db, user.id, start, end)
    recent = fetch_transactions(db, user.id, limit=RECENT_LIMIT, start=start, end=end)

    def amount_of(t: stats.TypeTotal | None) -> Decimal:
