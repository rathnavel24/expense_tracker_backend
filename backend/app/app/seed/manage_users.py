...
Usage (from backend/app/, with the virtualenv active):
...
def main() -> None:
    parser = argparse.ArgumentParser(prog="python -m app.seed.manage_users")
        ...
        try:
            if args.command == "create-user":
                user = auth_service.create_user(
                    db, email=args.email, name=args.name, password=password
                )
                print(f"Created user {user.email}.")
            else:
                auth_service.set_password(db, email=args.email, password=password)
